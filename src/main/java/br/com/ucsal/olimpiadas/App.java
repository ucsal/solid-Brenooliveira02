...
public class App {
	...
	static void aplicarProva() {
		...
		System.out.println("\n--- Fim da Prova ---");
		System.out.printf("Nota (acertos): %d / %d%n",
				tentativa.calcularNota(), tentativa.getRespostas().size());
	}
	...
	static Long escolherParticipante() {
		...
		return lerIdValido(participantes.stream().map(Participante::getId).toList());
	}
	...
	static Long escolherProva() {
		...
		return lerIdValido(provas.stream().map(Prova::getId).toList());
	}


	private static Long lerIdValido(List<Long> idsValidos) {
		try {
	...
	private static void imprimirTabuleiro(String fen) {
